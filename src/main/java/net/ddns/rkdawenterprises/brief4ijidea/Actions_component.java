            ...
            for( Node node : iterable( keymap_node.getChildNodes() ) )
            {
                if( node != null )
                {
                    if( node.getNodeType() == Node.COMMENT_NODE )
                    {
                        ...
                        if( last_comment_text.contains( s_documentation_key ) )
                        {
                            int start = last_comment_text.indexOf( s_documentation_key ) + s_documentation_key.length();
                            m_documentation_URI = last_comment_text.substring( start )
                                                                   .trim();
                        }
                    }
    ...
    private static void add_actions( @NotNull Map<String, Keymap_action_data> keymap_file )
    {
        ...
        for( Map.Entry<String, Keymap_action_data> keymap_file_entry : keymap_file.entrySet() )
        {
            ...
            if( action_ID.startsWith( "net.ddns.rkdawenterprises.brief4ijidea.actions." ) )
            {
                ...
                {
                    System.out.println( "brief4ijidea.Actions_component.add_actions: Error " + exception.getLocalizedMessage() );
                }
            }
        }
    }
